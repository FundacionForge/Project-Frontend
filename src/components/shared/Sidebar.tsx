          ...
          <div className="flex flex-col justify-between h-[800px]">
            <nav>
              ...
              <ul>
                {registers.map((reg) => (
                  <li key={reg.to}>
                    <Link to={reg.to} className="flex items-center dark:text-gray-300 gap-4 p-2 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors rounded-lg">{reg.icon} <span>{reg.name}</span></Link>
                  </li>
                ))}
              </ul>
